...
public class PlanItemController {
    ...
    @GetMapping("/{id}")
    public PlanItemResponse findById(@PathVariable Long id) {
        return service.findById(id);
    }

    @PostMapping
    public ResponseEntity<PlanItemResponse> create(@Valid @RequestBody PlanItemRequest request) {
        PlanItemResponse response = service.create(request);
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }

    @PutMapping("/{id}")
    public PlanItemResponse update(@PathVariable Long id, @Valid @RequestBody PlanItemRequest request) {
